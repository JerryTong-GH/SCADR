...
if nargin > 2
    ...
    okargs = {'smoothing','bins','plottype','logy','marker','msize','filled','axes_handle'};
    for j=1:2:nargin-2
        pname = varargin{j};
        pval = varargin{j+1};
...
elseif length(k)>1
    ...
    switch(k)
        case 1 %'surf'
            h = surf(axes_handle,ctrs1,ctrs2,F,'edgealpha',0);
        case 2 % 'mesh'
            h = mesh(axes_handle,ctrs1,ctrs2,F);
        case 3 %'contour'
            [dummy, h] =contour(axes_handle,ctrs1,ctrs2,F);
        case 4 %'image'
            nc = 256;
            F = F./max(F(:));
            colormap(repmat(linspace(1,0,nc)',1,3));
            h =image(axes_handle,ctrs1,ctrs2,floor(nc.*F) + 1);
        case 5 %'scatter'
            F = F./max(F(:));
            ind = sub2ind(size(F),bin(:,1),bin(:,2));
            col = F(ind);
            if filled
                h = scatter(axes_handle,X,Y,msize,col,'s','filled');
            else
                h = scatter(axes_handle,X,Y,msize,col,'s');
            end
            h.Marker = marker;
    end
end
