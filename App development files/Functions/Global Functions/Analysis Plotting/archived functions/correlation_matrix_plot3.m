function [] = correlation_matrix_plot3(total_data_sets,new_column_names,cell_line_names, ...
    do_correlationmatrix_plot_expression,do_correlationmatrix_plot_normalise,do_linear_fit, ...
    expression_channel_name,normalising_channel_name,cell_variant_plot,scatterbins, ...
...
if ~do_correlationmatrix_plot_normalise
    ...
    % Create plotmatrix
    fig = figure('Name', cell_variant_plot,'Position',[200,100, 1000, 1000]);
    ...
    for y = 1:nVars
        for x = 1:nVars
            ...
            if y==x
                ...
                text(min(xlim)+range(xlim)*.05,max(ylim),...
                    sprintf('Mu=%.3f, std=%.3f',mean(data(:,x)),std(data(:,x))),'Horiz','Left','Vert','top','FontSize',10,'FontWeight','bold','Color','r')
            else
                %Scatter plot
                ...
                text(min(xlim)+range(xlim)*.05,max(ylim),...
                    sprintf('r=%.2f, p=%.2f',r(1,2),p(1,2)),'Horiz','Left','Vert','top','FontSize',10,'FontWeight','bold','Color','r')
                if do_linear_fit
